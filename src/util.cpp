...
unsigned char Util::hexConv(char t)
{
    if ((t >= '0') && (t <= '9'))
    {
...
std::string Util::urlDecode(const std::string& st)
{
    ...
    while (ptr < end)
    {
        switch (*ptr)
        {
            case '%':
            {
                r << static_cast<char>((hexConv(*(ptr + 1)) << 4) + (hexConv(*(ptr + 2))));
                ptr += 3;
                break;
            }
            case '+':
...
std::string Util::urlEncode(const std::string& st)
{
    ...
    for (size_t i = 0; i < st.length(); i++)
    {
        ...
        else
        {
            ...
            r << "%" << lookupTable[t >> 4] << lookupTable[t & 0xf];
        }
    }
    return r.str();
}
...
std::vector<string> Util::extract(const std::string& st)
{
    ...
    for (curPos = 0; curPos < st.size();)
    {
        ...
        {
            if (static_cast<unsigned char>(st[valBegin]) > ' ') break;
        }
        if (valBegin != st.size())
        {
            if ((st[valBegin] == '\"') || (st[valBegin] == '\''))
            {
                ...
                {
                    if (st[valEnd] == st[valBegin]) break;
                }
                curPos = valEnd + 1;
                valBegin++;
            }
            else
            {
                for (valEnd = valBegin + 1; valEnd < st.size(); valEnd++)
                {
                    if (static_cast<unsigned char>(st[valEnd]) <= ' ') break;
                }
                curPos = valEnd;
            }
...
std::string Util::makeHttpTime(time_t t)
{
    ostringstream r;
    tm* tms = gmtime(&t);
    if (tms->tm_mday < 10) r << "0";
    r << tms->tm_mday << " ";
    r << months[tms->tm_mon] << " ";
    r << tms->tm_year + 1900 << " ";
    if (tms->tm_hour < 10) r << "0";
    r << tms->tm_hour << ":";
    if (tms->tm_min < 10) r << "0";
    r << tms->tm_min << ":";
    if (tms->tm_sec < 10) r << "0";
    r << tms->tm_sec << " GMT";
    return r.str();
}
...
time_t Util::parseHttpTime(const std::string& httpTime)
{
    ...
    if (commaPos != string::npos) t = t.substr(commaPos + 2);
...
std::string Util::utf8Encode(const std::wstring& st)
{
    ...
    for (size_t i = 0; i < st.length(); i++)
    {
        ...
        else
        {
            ...
            {
                r << static_cast<char>(((t >> 12)) | 0xE0)
                  << static_cast<char>(((t >> 6) & 0x3F) | 0x80)
                  << static_cast<char>((t & 0x3F) | 0x80);
            }
        }
    }
    return r.str();
...
std::wstring Util::utf8Decode(const std::string& st)
{
    ...
    for (i = 0; i < st.size();)
    {
        if (st[i] & 0x80)
        {
            if (st[i] & 0x20)
            {
                if (i >= (st.size() - 2)) throw runtime_error("UTF8 decoding error");
                t = (static_cast<unsigned int>(st[i] & 0x0F) << 12)
                    | (static_cast<unsigned int>(st[i + 1] & 0x3F) << 6)
                    | (static_cast<unsigned int>(st[i + 2] & 0x3F));
                r += static_cast<wchar_t>(t);
                i += 3;
            }
            else
            {
                if (i >= (st.size() - 1)) throw runtime_error("UTF8 decoding error");
                t = (static_cast<unsigned int>(st[i] & 0x1F) << 6)
                    | (static_cast<unsigned int>(st[i + 1] & 0x3F));
                r += static_cast<wchar_t>(t);
                i += 2;
            }
        }
...
std::string Util::mimeType(const std::string& ext)
{
    ...
    if (ext == "htm" || ext == "html") r = "text/html";
    if (ext == "xml") r = "text/xml";
    if (ext == "css") r = "text/css";
    if (ext == "js") r = "application/javascript";
    if (ext == "gif") r = "image/gif";
